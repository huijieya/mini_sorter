...
export default {
  name: 'ControlPanel',
  props: ['order', 'mode', 'feedback', 'cameraImg', 'lang'],
  ...
  <div class="h-full flex flex-col p-2.5 gap-2 overflow-hidden font-sans">
    <div class="flex items-center justify-between px-1">
      <span class="text-xs font-bold text-gray-400 uppercase tracking-tighter">{{ t('mode_label') }}</span>
    ...
    <div class="grid grid-cols-2 gap-x-2 gap-y-1 p-2 bg-white/5 rounded-lg border border-white/5 text-[10px]">
      <div class="flex gap-1 overflow-hidden"><span class="text-gray-500 shrink-0">{{ t('order_id') }}:</span><span class="text-gray-200 font-mono truncate leading-tight">{{ order.orderId || '-' }}</span></div>
      <div class="flex gap-1 overflow-hidden"><span class="text-gray-500 shrink-0">{{ t('barcode') }}:</span><span class="text-cyan-400 font-mono truncate font-bold leading-tight">{{ order.barcode || '-' }}</span></div>
      <div class="flex gap-1 col-span-2 border-t border-white/5 pt-1 mt-0.5"><span class="text-gray-500 shrink-0">{{ t('product_name') }}:</span><span class="text-gray-100 font-bold truncate leading-tight">{{ order.name || '-' }}</span></div>
      <div class="flex gap-1"><span class="text-gray-500 shrink-0">{{ t('required') }}:</span><span class="text-cyan-400 font-black leading-tight">{{ order.required || 0 }}</span></div>
    </div>

    <div class="flex items-center justify-between px-1 py-1 border-y border-white/5">
      <span class="text-[11px] font-black text-gray-100 uppercase tracking-tighter">{{ t('actual_count') }}</span>
    ...
    <div class="flex-1 min-h-0 relative border border-white/5 rounded-xl bg-black/50 flex items-center justify-center overflow-hidden group shadow-inner">
      ...
      <div v-else class="flex flex-col items-center gap-1 opacity-10 group-hover:opacity-25 transition-all duration-500 scale-90">
        ...
        <span class="text-[7px] font-black tracking-[0.3em] uppercase">{{ t('camera_standby') }}</span>
      </div>
